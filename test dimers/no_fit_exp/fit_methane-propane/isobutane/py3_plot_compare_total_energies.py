...
fig.text(0.5,0.035, 'DFT-SAPT Energy / kJ/mol',ha='center',
        va='center',
        fontweight='semibold',
        fontsize=24)
...
first=True
labels = []
label_texts = itertools.cycle(['Isobutane','Aniso-Iso FF','MASTIFF'])
...
for i, (component_prefix,component_suffix) in enumerate(zip(component_prefixes,component_suffixes)):
    ...
    for ic,component in enumerate([total_energy,total_energy]):
        ...
        ax.set_ylim(lims)
        if ic == 0:
            ax.set_title(next(label_texts),fontweight='bold',
                    color=palette[i],
                    #color='k',
                    fontsize=24)
